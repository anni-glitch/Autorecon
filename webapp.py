...
async def login_for_access_token(request: Request, response: Response, username: str = Form(...), password: str = Form(...), db: Session = Depends(database.get_db)):
    ...
    user = db.query(models.User).filter(func.lower(models.User.username) == func.lower(username)).first()
    ...
    response.set_cookie(key="access_token", value=access_token, httponly=True, samesite="lax", secure=is_secure)
    response.set_cookie(key="refresh_token", value=refresh_token, httponly=True, samesite="lax", secure=is_secure)
...
async def register_user(request: Request, response: Response, username: str = Form(...), password: str = Form(...), db: Session = Depends(database.get_db)):
    ...
    from sqlalchemy import func
    existing = db.query(models.User).filter(func.lower(models.User.username) == func.lower(username)).first()
    if existing:
...
async def refresh_token(response: Response, refresh_token: Optional[str] = Cookie(None), db: Session = Depends(database.get_db)):
    ...
    # Note: request is not passed to refresh_token by default, but we can detect via header or just allow False for refresh
...
async def export_job(job_id: int, format: str = "json", current_user: models.User = Depends(auth.get_current_user), db: Session = Depends(database.get_db)):
    ...
    if format == "pdf":
        ...
        severity_style = lambda s: ParagraphStyle('Sev', parent=styles['Normal'], fontSize=9, fontName='Helvetica-Bold', textColor=colors.HexColor('#ef4444' if s=='Critical' else '#f97316' if s=='High' else '#eab308' if s=='Medium' else '#3b82f6' if s=='Low' else '#64748b'))

        elements = []
        # Header Box
        header_table = Table([
            [Paragraph(f"<b>Security Scan Report: {job.target}</b>", title_style)],
            [Paragraph(f"Scan ID: {job.id} | Date: {job.created_at.strftime('%Y-%m-%d %H:%M:%S')}", styles['Normal'])]
        ], colWidths=[450])
        ...
        if not findings:
            ...
            for f in findings:
                ...
                sev_color = colors.HexColor('#ef4444' if f.severity=='Critical' else '#f97316' if f.severity=='High' else '#eab308' if f.severity=='Medium' else '#3b82f6' if f.severity=='Low' else '#64748b')
                
                data = [[Paragraph(f"<b>{f.severity.upper()}</b>", severity_style(f.severity)), 
                         Paragraph(f"<b>[{f.module}] {f.title}</b>", finding_title_style)]]
                
                t = Table(data, colWidths=[70, 380])
                t.setStyle(TableStyle([
                    ('VALIGN', (0, 0), (-1, -1), 'TOP'),
                    ('LINEBELOW', (1, 0), (1, 0), 1, colors.HexColor('#e2e8f0')),
...
async def start_scan(
    ...
    # Sanitize target (strip protocol and trailing slash)
